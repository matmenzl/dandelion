	...
	*********************************************************************************/
	...
	switch ($auth_info['type']) {
		...
		case 'live':
			...
			echo '<p class="wpstagecoach-textinfo">' . __(' Staging site: ', 'wpstagecoach' ) . '&nbsp;<a href="http://' . $auth_info['stage-site'] . '" target="_blank">' . $auth_info['stage-site'] . '</a><br/>';
			echo __( 'Staging site log in: ', 'wpstagecoach' ) . '<a target="_BLANK" href="http://' . $auth_info['stage-site'] . '/wp-admin/">' . $auth_info['stage-site'] . '/wp-admin/</a> ';
	...
	*********************************************************************************/
	...
	if( stripos( site_url(), 'https' ) !== false || stripos( get_option('siteurl'), 'https' ) !== false || isset($_SERVER['HTTPS']) ){
		if( isset( $wpsc['subscription'] ) && strpos( $wpsc['subscription'], 'single-site' ) !== false ){
			...
			$errmsg .= '<p>' . __( 'Your site is encrypted using SSL (https), however, your plan (' . ucwords( str_replace( "-", " ", $wpsc['subscription'] ) ) . ') does not include SSL encryption on the staging site.', 'wpstagecoach' ) . '</p>';
			$errmsg .= __( 'You will almost certainly run into problems while trying to use the staging site on WP Stagecoach because it will not have SSL, but if you know what you are doing, you may continue.', 'wpstagecoach' ) . '</p>';
			$errmsg .= '<p>' . __( 'Please visit <a href="https://wpstagecoach.com/your-account/#subscription" target="_blank">Your Account</a> page to change plans to one that <a href="https://wpstagecoach.com/pricing" target="_blank">supports SSL</a>.', 'wpstagecoach' ) . '</p>';
		} else {
...
function wpsc_check_post_info($action, $url, $post_args, $post_result, $display_output=true){
	...
	*********************************************************************************/
	...
	if( is_array( $post_result ) && !empty( $post_result['body'] ) ){
		...
		if( $post_result['response']['message'] != 'OK' ){ // make sure we got an OK response.
			$errmsg  = '<p>' . __( 'We got a bad HTTP response from the server url: ', 'wpstagecoach' ) . $url .'<br/>'.PHP_EOL;
			if( isset( $post_result['response']['code'] ) )
				$errmsg .= __( 'Error code: ', 'wpstagecoach' ) . $post_result['response']['code'] . '</b></p>' . PHP_EOL;
			$error = true;
...
function wpsc_display_feedback_form($location, $site_info, $message='', $step=false ){
	...
	*********************************************************************************/
	...
	if( strstr($location, 'error') !== false ){
		...
		$output .= '<label>' . __( 'Comments:', 'wpstagecoach' ) . '</label><br/>'.PHP_EOL;
	}
