...
define( 'WPSTAGECOACH_VERSION',				'1.3.5' );
...
// old thing
// <input type="text" size="40" name="wpsc-apikey" value="<?php echo ( isset( $wpsc['apikey'] ) ? $wpsc['apikey'] : '' ); 
// (cont) ? >"/>'
						?>
					</td>
				</tr>
				...
				<tr><th valign="top"><label for="wpsc-advanced"><?php _e( 'Enable Advanced Options?', 'wpstagecoach' ); ?></label></th>
					<td valign="top">
				...
				<tr>
					<th valign="top"><label for="wpsc-slow"><?php _e( 'Optimize WP Stagecoach for a slower server?', 'wpstagecoach' ); ?></label></th>
					<td valign="top">
...
function wpstagecoach_debug() {
	...
	if( isset( $_POST['wpsc-debug-settings'] ) ) {
		...
		if( isset( $_POST['wpsc-show-debug-messages'] ) && $_POST['wpsc-show-debug-messages'] == 'on' ){
			...
			_e( '<p>Not updating debug messages option to the same option.</p>', 'wpstagecoach' );
			$update_debug = false;
		} else {
